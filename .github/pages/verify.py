...
def verify(output, evidence):
    ...
    server = ThreadingHTTPServer(
        ("127.0.0.1", 0),
        partial(Handler, directory=str(output.resolve())),
    )
            ...
            for name in config["pages"]:
                ...
                for width in [1440, 390]:
                    ...
                    if not route and "screenshots/index.html" in config["pages"]:
                        ...
                        assert strips.count() > 0, (
                            "Homepage must use its screenshot gallery"
                        )
                        for strip in strips.all():
                            ...
                            expect(
                                strip.locator("a:not(.abx-marquee-card)"),
                            ).to_have_count(0)
                            ...
                            if overflowing:
                                ...
                                assert (
                                    viewport.evaluate("node => node.scrollLeft")
                                    == stopped
                                )
                                ...
                                assert centered, (
                                    "Short screenshot strips must be centered"
                                )
                            for card in cards.all():
                                ...
                                if anchor:
                                    assert f'id="{anchor}"' in response.text(), (
                                        f"Missing screenshot anchor: {href}"
                                    )
                    ...
                    images = page.locator("main img").evaluate_all(
                        "nodes => [...new Set(nodes.map(image => image.src))]",
                    )
                    local_images = [
                        url for url in images if url.startswith(origin + "/")
                    ]
                    page.evaluate(
                        """async urls => {
                    ...
                    page.locator(".abx-footer").screenshot(
                        path=str(
                            evidence
                            / f"{route.replace('/', '-') or 'home-'}footer-{width}.png",
                        ),
                    )
                    for link in header.locator(".abx-nav > a").all():
                        ...
                        if url.fragment:
                            body = response.text()
                            assert (
                                f'id="{url.fragment}"' in body
                                or f"id='{url.fragment}'" in body
