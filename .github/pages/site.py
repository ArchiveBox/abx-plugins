...
def render(output, source=None, baseurl=""):
    ...
    for name in config["pages"]:
        ...
        header = (
            (HERE / "base/header.html")
            .read_text()
            .replace("__SITE_NAME__", escape(config["name"]))
        )
        ...
        for kind in ["FOOTER-START", "FOOTER-END"]:
            text = replace_block(
                text,
                kind,
                (HERE / "base" / f"{kind.lower()}.html").read_text().strip(),
            )
        text = text.replace("__SITE_BASE__", base).replace(
            "__SITE_REVISION__",
            revision,
        )
...
def verify(output, baseurl=""):
    ...
    for name in config["pages"]:
        ...
        if (page.headers, page.footers, page.canonicals) != (1, 1, 1):
            raise ValueError(
                f"{name}: expected exactly one shared header, footer and canonical",
            )
        if "__SITE_" in text:
        ...
        for resource in page.resources:
            ...
            local = (
                output / resource_path.lstrip("/")
                if resource_path.startswith("/")
                else path.parent / resource_path
            ).resolve()
        ...
        for asset in ["chrome.css", "chrome.js"]:
            if (output / "site-base" / asset).read_bytes() != (
                HERE / "base" / asset
            ).read_bytes():
                raise ValueError(f"{name}: stale common asset {asset}")
    print(
        f"Verified shared layout and local resources for {len(config['pages'])} pages",
    )
