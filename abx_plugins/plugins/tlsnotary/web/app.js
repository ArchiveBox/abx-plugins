...
async function verify(file) {
  ...
  status.textContent='Checking signature, certificate, timestamp and complete response…';
  try {
    await newWorker();
    ...
    status.textContent='Valid signature and authenticated response.';status.className='valid';
    ...
    for(const [label,value] of Object.entries({URL:checked.url,'TLS connection':new Date(checked.connection_time_unix*1000).toISOString(),'HTTP status':checked.status,'Content type':checked.content_type,'Body bytes':body.length,'SHA-256':checked.body_sha256})) {
