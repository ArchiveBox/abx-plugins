...
impl Context {
        ...
        R: Send + 'static,
    {
        ...
        // Bound queued channels independently of machine speed, matching the
        // 32-way bound introduced by upstream mpz's newer executor.
        let mut results = Vec::with_capacity(items.len());
        let mut items = items.into_iter().enumerate().peekable();
        while items.peek().is_some() {
            let mut tasks = Vec::new();
            for (i, item) in items.by_ref().take(32) {
                let i = u32::try_from(i).expect("more than u32::MAX items");
                let mut ctx = self.child(parent_id.child(i))?;
                let f = f.clone();
                tasks.push(run(executor.as_ref(), async move { f(&mut ctx, item).await }));
            }
            results.extend(future::join_all(tasks).await);
            // Local completion alone does not bound the peer's unconsumed streams.
            // Wait until both peers have consumed this batch before opening more.
            let completed = results.len() as u64;
            self.io.send(completed).await.map_err(ContextError::mux)?;
            let peer: u64 = self.io.expect_next().await.map_err(ContextError::mux)?;
            if peer != completed {
                return Err(ContextError::mux(std::io::Error::new(
                    std::io::ErrorKind::InvalidData, "MPC map batch mismatch",
                )));
            }
        }
        Ok(results)
