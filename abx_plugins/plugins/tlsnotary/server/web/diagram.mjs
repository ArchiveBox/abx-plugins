...
if (lab && !new URLSearchParams(location.search).has('compact')) {
  ...
  const events = [
    ...
    { phase:'Response', route:['client'], kind:'response', label:'200 OK …', detail:'Readable locally', title:'Assemble the local response transcript', copy:'The complete readable response is now on the client. Watch it appear in the worksheet. These are the bytes later saved as response.http.', payload:response, visibility:'This content stays with the client. It is not sent to the verifier to obtain the receipt.' },
  ...
  const entries = [
    ...
    {at:16,source:'joint',label:'Commitment proof · in progress',text:()=> 'Private response + blinder\n    ↓ hidden computation\nSHA256 commitment\n    ↓ proof checked by Verifier'},
    ...
    {at:19,source:'client',label:'Add the local opening',text:()=> 'Receipt received + signature checked.\nAdd the 16-byte blinder kept by Client.\nThe verifier never sent the blinder.'},
    {at:20,source:'client',label:'Saved locally',text:()=> 'response.http + receipt.json\nResponse body stored once.\nNo separate full proof transcript needed.'},
  ...
  function renderEvidence(index,progress) {
    ...
    $('#evidence-stage').textContent=index<5?'Collecting metadata':index<14?'Collecting TLS records':index<16?'Opening response locally':index<18?'Proving the commitment':index<20?(index===18?'Verifier signs receipt':'Receiving signed receipt'):'Files ready';
    ...
    $('#evidence-note').textContent=index>=18
      ...
      : 'This is a temporary worksheet on the client, not a proof file uploaded to the verifier.';
