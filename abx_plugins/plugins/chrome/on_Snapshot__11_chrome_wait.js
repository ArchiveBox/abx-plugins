...
async function main() {
  ...
  console.error(
    `[chrome_wait] Waiting for Chrome session (timeout=${timeoutSeconds}s)...`
  );
  ...
  console.error(
    `[chrome_wait] Chrome session ready (verified CDP connection, cdp_url=${cdpUrl.slice(
      0,
      32
    )}..., target_id=${targetId}).`
  );
  ...
  emitArchiveResultRecord(
    "succeeded",
    `tab ready target=${targetId} port=${port}`
  );
  process.exit(0);
}
