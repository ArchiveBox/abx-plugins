...
function emitProgress(force = false) {
  const line = `${errorCount} console error${
    errorCount === 1 ? "" : "s"
  ...
  if (
    force ||
    lastProgressAt === 0 ||
    now - lastProgressAt >= PROGRESS_DEBOUNCE_MS
  ) {
  ...
  if (!pendingProgressTimer) {
    pendingProgressTimer = setTimeout(
      emitLine,
      PROGRESS_DEBOUNCE_MS - (now - lastProgressAt)
    );
  }
}
...
async function setupListeners() {
  ...
  // treat its existence as a real readiness signal before navigation.
...
function emitResult(
  status = "succeeded",
  outputStr = `${errorCount} errors | ${warningCount} warnings`
) {
...
async function main() {
  ...
  if (!getEnvBool("CONSOLELOG_ENABLED", true)) {
    ...
    emitArchiveResultRecord("skipped", "CONSOLELOG_ENABLED=False");
    process.exit(0);
  }
