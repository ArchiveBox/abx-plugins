...
def test_prefers_dom_output_over_singlefile_when_both_exist(
    ...
    binary_path = require_trafilatura_binary()
    snap_dir = real_competing_html_snapshot(tmp_path, "trafilatura-precedence")
    env = os.environ.copy()
    env["SNAP_DIR"] = str(snap_dir)
    env["TRAFILATURA_BINARY"] = binary_path
    result = subprocess.run(
        [str(TRAFILATURA_HOOK), "--url", TEST_URL],
        cwd=tmp_path,
        capture_output=True,
        text=True,
        timeout=120,
    ...
    content_txt = (snap_dir / "trafilatura" / "content.txt").read_text().lower()
    assert "example domain" in content_txt
    assert "archivebox" not in content_txt
