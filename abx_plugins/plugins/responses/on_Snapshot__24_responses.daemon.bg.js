...
function emitResponseProgress(force = false) {
  const line = `${responseCount} response${
    responseCount === 1 ? "" : "s"
  } captured`;
  if (force) {
    ...
    try {
      ...
      if (isHtmlResponse) {
        ...
        if (
          ...
          (bodySize === preferredOutputSize &&
            (!preferredOutputPath || relativeOutputPath < preferredOutputPath))
        ) {
...
async function main() {
  ...
  try {
    ...
    emitResponseProgress(true);
    emitProcessReadyRecord({ plugin: PLUGIN_DIR, output: "index.jsonl" });
