...
def main(
    ...
    abx_pkg_args = _parse_extra_hook_args(ctx.args if ctx else [])
    ...
    puppeteer_binary = Binary(**puppeteer_kwargs).load_or_install()  # ty:ignore[invalid-argument-type]
    ...
    if proc.returncode != 0 and _should_repair_puppeteer_install(
        ...
        puppeteer_binary = Binary(**puppeteer_kwargs).load()  # ty:ignore[invalid-argument-type]
        proc = _run_puppeteer_install(
            binary=puppeteer_binary,
            install_args=install_args,
            cache_dir=cache_dir,
...
def _parse_override_install_args(
    ...
    if (
        not isinstance(provider_overrides, dict)
        or "install_args" not in provider_overrides
    ):
        return default
    install_args = provider_overrides["install_args"]  # ty:ignore[invalid-argument-type]
