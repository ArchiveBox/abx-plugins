 ...
 *
 * Usage: on_Snapshot__25_redirects.daemon.bg.js --url=<url>
...
function appendRedirectEntry(outputPath, entry) {
  ...
  const redirectCount = redirectChain.filter(
    (redirect) => redirect.type !== "initial"
  ).length;
  const progressLine =
    redirectCount === 0
      ? "0 redirects"
...
async function handleShutdown(signal) {
  ...
  const result = {
    ...
    is_redirect:
      redirectChain.length > 0 || (finalUrl && finalUrl !== originalUrl),
  };
...
async function main() {
  ...
  if (!url) {
    console.error("Usage: on_Snapshot__25_redirects.daemon.bg.js --url=<url>");
    process.exit(1);
  }
